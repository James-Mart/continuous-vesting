...
impl RewardBucket {
    ...
    fn claim(&mut self) -> u128 {
        let amt = self.balance_claimable();
        self.total_claimed = self.total_claimed.saturating_add(amt);
        amt
    }

    /// Optional convenience if you ever want partial claims.
    fn claim_up_to(&mut self, max: u128) -> u128 {
        let amt = self.balance_claimable().min(max);
        self.total_claimed = self.total_claimed.saturating_add(amt);
        amt
    }

    /// Total still in the bucket (unclaimed).
